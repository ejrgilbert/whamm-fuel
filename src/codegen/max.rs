...
use crate::codegen::{handle_reqs, process_needed_state, CodeGenResult, GeneratedFunc, ReqState, StackVal};
...
fn gen_func<'a, 'b>(true_start_idx: usize, spec_name: &str, cost_map: &mut HashMap<usize, u64>, orig_fid: u32, body: &[Operator<'a>], slice: &Slice, func_slices: &SliceResult, ty: &CompType, gen_wasm: &mut Module<'b>, generated_funcs: &mut Vec<GeneratedFunc>) where 'a: 'b {
    let (mut state, used_params) = CodeGenMax::new(slice);     // one instance of state per function!
...
fn calc_op_cost(is_in_slice: bool, at_func_end: bool, op: &Operator, state: &mut CodeGenMax) -> bool {
...
fn gen_fuel_comp(fuel: &LocalID, ty: &CompType, state: &mut CodeGenMax, func: &mut FunctionBuilder) {
    match ty {
...
fn gen_fuel_comp_exact(fuel: &LocalID, state: &mut CodeGenMax, func: &mut FunctionBuilder) {
...
fn gen_fuel_comp_approx(_fuel: &LocalID, _state: &mut CodeGenMax, _func: &mut FunctionBuilder) {
    // TODO
    todo!()
}

// Translate instructions into `local.get` on parameter representing that state! (if necessary)
fn gen_op<'a, 'b>(opidx: usize, op: &Operator<'a>, fuel: &LocalID, gen_state: &CodeGenMax, func: &mut FunctionBuilder<'b>) where 'a : 'b {
