...
use crate::codegen::{handle_reqs, process_needed_state, CodeGenResult, GeneratedFunc, ReqState};
use crate::run::CompType;
use crate::slice::{Slice, SliceResult};
use crate::utils::is_branching_op;

#[derive(Default)]
pub(crate) struct CodeGenMin {
    pub(crate) fid: u32,
    ...
    pub for_taken: HashMap<usize, ReqState>,
...
pub fn codegen_min<'a, 'b>(ty: &CompType, slices: &mut [SliceResult], funcs: &[FuncState], wasm: &Module<'a>, gen_wasm: &mut Module<'b>) -> CodeGenResult where 'a : 'b {
...
fn gen_func<'a, 'b>(true_start_idx: usize, spec_name: &str, cost_map: &mut HashMap<usize, u64>, orig_fid: u32, body: &[Operator<'a>], slice: &Slice, func_slices: &SliceResult, ty: &CompType, gen_wasm: &mut Module<'b>, generated_funcs: &mut Vec<GeneratedFunc>) where 'a: 'b {
    let (mut state, used_params) = CodeGenMin::new(slice);     // one instance of state per function!
...
fn calc_op_cost(is_in_slice: bool, at_func_end: bool, op: &Operator, state: &mut CodeGenMin) -> bool {
...
fn gen_fuel_comp(fuel: &LocalID, ty: &CompType, state: &mut CodeGenMin, func: &mut FunctionBuilder) {
    match ty {
...
fn gen_fuel_comp_exact(fuel: &LocalID, state: &mut CodeGenMin, func: &mut FunctionBuilder) {
...
fn gen_fuel_comp_approx(_fuel: &LocalID, _state: &mut CodeGenMin, _func: &mut FunctionBuilder) {
    // TODO
    todo!()
}

// Translate instructions into `local.get` on parameter representing that state! (if necessary)
fn gen_op<'a, 'b>(opidx: usize, op: &Operator<'a>, fuel: &LocalID, gen_state: &CodeGenMin, func: &mut FunctionBuilder<'b>) where 'a : 'b {
    if handle_reqs(gen_state.for_taken.get(&opidx), func) {
    }
